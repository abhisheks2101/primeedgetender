"""Uttar Pradesh government tender collector."""
...
from app.collectors.up.up_client import UPPortalClient
from app.collectors.up import up_parsers
from app.core.enums import TenderStatus


class UPTenderCollector(TenderCollector):
    """Collects publicly listed UP tenders from the NIC GeP home page and detail pages."""
    ...
    def __init__(
        self,
        *,
        client_factory: type[UPPortalClient] | None = None,
        transport=None,
    ) -> None:
        self._client_factory = client_factory or UPPortalClient
        self._transport = transport
        self._client: UPPortalClient | None = None

    async def _get_client(self, context: CollectionContext) -> UPPortalClient:
    ...
    async def discover(self, context: CollectionContext) -> DiscoveryResult:
        ...
        summaries = up_parsers.parse_listing_page(html, client.listing_page_url())
    ...
    async def fetch_details(self, source_tender_id: str, context: CollectionContext) -> RawTenderData:
        if context.current_detail_payload and context.current_detail_payload.get("source_tender_id") == source_tender_id:
            return RawTenderData(source_tender_id=source_tender_id, payload=context.current_detail_payload)
        ...
        if not detail_url:
            raise ParsingCollectionError(f"Could not resolve detail URL for tender {source_tender_id}.")

        html = await client.fetch_detail_page(detail_url)
        detail = up_parsers.parse_tender_detail(html, detail_url)
        payload = {
    ...
    def normalize(self, raw: RawTenderData, context: CollectionContext) -> NormalizedTenderDraft:
        ...
        status, source_status = up_parsers.parse_status(submission_end)
        ...
        return NormalizedTenderDraft(
            ...
            state=detail.get("state") or "Uttar Pradesh",
